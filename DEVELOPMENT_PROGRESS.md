...
### No-clear-face source spike recovery candidate (2026-09-25)

The CN3_92 trace showed that two source-visible full spikes were generated by
`_detect_geometry`, then both were removed in `_reconcile_common_room_geometry`
because every candidate terrain face was occupied. At `(288,96)` the same
patch also produced a weak block hypothesis; the source-specific down-triangle
score is 1.0, classifier margin 0.327, and outline delta 0.587. At `(288,160)`
the corresponding up-triangle values are 1.0, 0.385, and 0.567. Both are
separate expected objects in the frozen region.

Candidate v10 adds one narrow shared rule: only in the no-clear-face branch,
retain a spike if its directed source template is at least 11/12, competing
directions are at most 1/3, and the local classifier agrees with margin at
least 0.20 and outline delta at least 0.30. This does not pick a replacement
coordinate or palette. The later source/material arbiter still decides
whether any overlapping block is intrusive; if source colors cannot separate
front and backing, it leaves the ambiguous block hypothesis intact. Unit
coverage passes for all four directions, both contrast polarities, and a
simulated 1.25x capture normalized to scanner coordinates; absent and
wrong-direction triangles do not receive the exemption.

The ordinary CN3_92 whole map changes at exactly five typed origins versus
v9: it adds the expected up `(3,288,160)` and down `(6,288,96)` and removes
three excess blocks `(1,288,96)`, `(1,296,96)`, `(1,296,160)`. Its frozen
structure moves from **50/52 exact, 5 extras, 2 misses** to **52/52 exact,
2 extras, 0 misses, 0 shifts and 0 wrong directions**. CN3_27 is map-identical
to v9 at **7/12 exact, 5 extras, 5 shifts, 0 misses and 0 wrong directions**;
the predeclared no-extra gate still fails there. This is a positive CN3_92
development result, not a completed batch or arbitrary-tileset guarantee.

The focused source-arbitration module passes **16 tests**. All16 exact-control
ordinary scans, quiet runtime comparison, final source/JTool/Blend visual
review and app restart/source-parity verification are pending for this
candidate; do not label it accepted or treat it as the final all-71 code.
Current candidate outputs and review SVGs are ignored under
`.artifacts/native-conflicts-20260923/current-candidate-v10/`. The source image,
